...
static sp_IntArray*sp_IntArray_from_step(mrb_int s,mrb_int e,mrb_int st){sp_IntArray*a=sp_IntArray_new();if(st==0)return a;if(st>0){for(mrb_int i=s;i<=e;i+=st)sp_IntArray_push(a,i);}else{for(mrb_int i=s;i>=e;i+=st)sp_IntArray_push(a,i);}return a;}
